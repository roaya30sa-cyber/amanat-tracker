...
export async function POST(req: NextRequest) {
  return handleAccess(async () => {
    ...
    const ins = await db.prepare(`
      INSERT INTO weekly_reports (region_id, report_date, current_task, priority, obstacles, solution_plan, required_resources, follow_up_date, created_at)
      VALUES (?,?,?,?,?,?,?,?,?) RETURNING *
    `).bind(
      regionId, body.report_date, body.current_task.trim(), body.priority ?? null,
